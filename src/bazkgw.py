...
def kimotogravitywell(blockindex, targetblocksspacingseconds, pastblocksmin, pastblocksmax):
    ...
    while True:
        ...
        if (diffiter == 1):                
            ...
            try:
                pastdifficultyaverage = ((getfromblockindex(blocklastsolved, 1, blockindex) - pastdifficultyaverageprev) / diffiter) + pastdifficultyaverageprev
            except:
                if debug is True:
        ...
        pastrateactualseconds = int(getfromblockindex(blocklastsolved, 0, blockindex)) - int(getfromblockindex(blockreading, 0, blockindex))
...
def getfromblockindex(count, blocktype, blockindex):

    #print 'complete blockindex [' + str(blockindex) + ']'
    print 'asked for count = ' + str(count),
    print 'of type = ' + str(blocktype)

    retrieved = ''
    try:
        signstring = 'BLK%s' % count
        for num in blockindex:
            if signstring in num:
                retrieved = num.replace('%s,' % signstring, '')
                break
    except:
        print 'blockindex error'
    if len(retrieved) > 0:
        return retrieved.split(',')[blocktype]
    else:
        return 'not found'
